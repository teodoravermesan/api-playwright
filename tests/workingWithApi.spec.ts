...
test('create from api and delete from ui', async ({ page, request }) => {
...
test('create and delete from api', async ({ page, request }) => {
  const articleResponse = await request.post('https://conduit-api.bondaracademy.com/api/articles/', {
    data: {
      "article": { "tagList": [], "title": "This is a test from teo", "description": "This is a test from teo", "body": "This is a test body" }
    }
  })
  expect(articleResponse.status()).toEqual(201)
  const articleResponseBody = await articleResponse.json()
